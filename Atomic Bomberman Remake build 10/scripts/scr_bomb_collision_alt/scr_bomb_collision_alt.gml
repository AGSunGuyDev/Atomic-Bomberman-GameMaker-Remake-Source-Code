...
function scr_bomb_collision_alt(){
			// Check for bomb position
		if (!place_meeting(x, y, Obj_Basic_Bomb)){
		    inside_bomb = false;
		}
		if (Can_Kick = false){
			if (!inside_bomb && place_meeting(x + xspd_alt, y + yspd_alt, Obj_Basic_Bomb)){
			    xspd_alt = 0;
				yspd_alt = 0;
			}
		}
		else if (Can_Kick = true){
			var _kickedbomb = instance_place(x + xspd_alt, y + yspd_alt, Obj_Basic_Bomb)
					...
					if (face == RIGHT){
						...
						if (_kickedbomb.kick_to_vertical = false){
							if (_kickedbomb.kick_to_side = true){
								...
								_kickedbomb.xspd_alt = 0;
							}
							//if the bomb is NOT in punched state, trigger kick state
							if (_kickedbomb.bomb_is_punched = false){
								_kickedbomb.kick_to_vertical = true;
								_kickedbomb.yspd_alt = -3.5;
								_kickedbomb.bomb_kick_trigger = true;
							}
						}
					}
				...
				scr_already_kicked_bomb_collision_alt();
		}
}